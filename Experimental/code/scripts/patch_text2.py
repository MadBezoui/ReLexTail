...
replacements = {
    "main.tex": [
        ...
        (r"ASF is thus LexPR's closest scalarisation rival.*?in practice\.", 
...
and the specified ASF baseline."""),
        ...
        (r"With a singleton probe \$q\$, LexPR reduces.*?lexicographic refinement\.",
...
refinement to the remaining sorted disappointments."""),
        ...
        (r"LexPR adopts the regret viewpoint but measures a dimensionless disappointment per\s+probe and aggregates these regrets lexicographically, rather than collapsing them\s+into a single worst-case scalar over a weight set\.",
...
substantive preference inputs."""),
        ...
        (r"LexPR keeps their goal of returning a defensible single choice but replaces those\s+numerical commitments with a declared family of monotone questions\.",
...
affect the resulting order."""),
        ...
        (r"We stress that the declared probes, their multiplicities, and their aggregation\s+forms are themselves preference inputs.*?scaling a coordinate as a scalar weight would\.",
...
must therefore be reported."""),
        ...
        # 3.3 Conclusion
        (r"LexPR is most appropriate when stakeholders can defend a finite set of monotone\s+questions, accept worst-first comparison across those questions, and can support\s+the governance required to freeze and version the shortlist, bounds, anchors,\s+and probe family\.",
...
shortlist, criterion bounds, probe anchors, and probe family."""),
        ...
        # 4.1 Introduction
        (r"In each case LexPR reports a labelled regret certificate that identifies the\s+binding probes\.",
...
needed to reconstruct the comparison."""),
        ...
        (r"\\caption\{From Pareto filtering to an auditable recommendation: the selected point\s+carries a sorted, labelled regret certificate.\}",
...
to reconstruct the comparison.}"""),
        ...
        (r"Because the recommendation is determined by the sorted vector of probe\s+disappointments.*?not appended\s+after the fact\.",
...
the exact LexPR preorder."""),
        ...
        (r"\\subsection\{The decision certificate\}\s+\\label\{sec:certificate\}\s+The recommendation is reported together with a formal certificate\.",
...
thresholds, and exact tie and tolerance conventions."""),
        ...
        (r"For the recommended \$x\$, order the labelled disappointments descending as.*?\\end\{equation\}",
...
tolerance-cycle conventions."""),
        ...
        # 4.7 Abstract
        (r"Numerical experiments on exact Pareto fronts and a reproducible\s+supplier-selection case indicate that LexPR is competitive.*?stability information\.",
...
does not establish general superiority."""),
        ...
        (r"LexPR always includes the \$m\$ singletons \(required for separation\), the grand\s+mean, and the grand maximum\.",
        ...
        (r"First the \\emph\{base\} questions: the \$m\$ singletons, mandatory for separation and\s+the strict Pareto guarantee, the grand mean as the utilitarian reading, and the\s+grand maximum as the egalitarian one\.",
...
already separates every criterion."""),
        ...
        (r"Second the \\emph\{redundancy\} structure: we recommend the threshold \$\\theta=0\.6\$\s+used throughout, followed by a sensitivity check at \$\\theta\\pm0\.3\$\.",
...
quality, and domain meaning of the criteria."""),
        ...
        (r"\\textbf\{Input:\} polyhedral feasible set \$\\X\$, affine criteria \$f_i\$, canonical\s+probes, tolerance \$\\tau\$\.\\\\",
         r"\textbf{Input:} polyhedral feasible set $\X$, affine criteria $f_i$, and a fixed retained family of canonical probes.\\"),

        # 17. Add K=0 branch
        (r"\\item Retain only probes with \$q\^\{\\mathrm w\}-q\^\\star>\\varepsilon\$, and flag\s+degenerate probes as non-informative for that run\.",
        ...
        (r"Because the tolerance relation \$\\prec_\\tau\$ of \\eqref\{eq:wtau\} is in general not transitive, computing \$W_\\tau\$ as the set of \$\\prec_\\tau\$-undominated alternatives requires pairwise comparisons\. With naive probe evaluation this costs.*?the third tests dominance\.",
...
For the canonical singleton, weighted-mean, and maximum probes evaluated naively, $C_{\Q}=O(Km)$, giving $O(NKm+NK\log K+NK)$."""),
        ...
        # 18.2 Theorem 8
        (r"With naive probe evaluation, computing \$W_0\$ requires.*?operations,",
r"""If $C_{\Q}$ is the cost of evaluating all retained probes for one candidate, computing $W_0$ requires
\[
O(NC_{\Q}+NK\log K+NK)
\]
operations. For the canonical probes under naive $O(m)$ evaluation per probe, this becomes
\[
O(NKm+NK\log K+NK)
\]
operations,"""),
...
use the term \emph{active bounds} when the distinction matters."""),
        ...
        (r"we provide an invariance condition and an interval-stability procedure that\s+reports possible and necessary winners under uncertain bounds\.",
...
probability."""),
        ...
        (r"Otherwise the \\emph\{certified selector\} returns the interval possible and\s+necessary winner sets\.",
        ...
        # 30. Runtime claim
        (r"As a rule of thumb, keeping \$K\\le 3m\$ holds the selection well under a second for\s+\$N\\le 1000\$ candidates on standard hardware\.",
        ...
        (r"Before final submission, the DOI and all scripts should be verified from a clean\s+environment, and the manuscript values should be checked automatically against\s+the archived \\texttt\{CSV\} and \\texttt\{JSON\} outputs\.",
...
\]"""),
        ...
        # 32. Supplement intro
        (r"This supplement collects the proofs omitted from the main text.*?Section~S5\)\.",
...
convergence checks, runtime results, and illustrative figures (Section~S5)."""),
        ...
        # 33. Caption
        (r"\\caption\{Adaptive probe construction.*?nontrivial clusters\.\}",
...
profile.}"""),
        ...
        # 34. Table S3
        (r"Criterion redundancy & Correlation cluster map and realised within-cluster.*?\\rho\.\s*\\\\",
        ...
        (r"Explanation can be slower & Short binding-probe certificate plus a full audit\s+certificate\.",
    ...
    "theory_core.tex": [
        ...
        (r"Real criteria are frequently redundant.*?Section~\\ref\{sec:supplier-ablation\}\.",
...
sensitivity analyses."""),
        ...
        (r"Redundancy is treated as positive correlation: two criteria that rise and fall\s+together carry duplicate information, whereas negatively correlated criteria\s+represent genuine trade-offs and must remain separate\.",
...
correlation coefficient."""),
        ...
        (r"Thus the discrepancy induced by summarising a redundant cluster by its mean is\s+controlled by the correlation gap \$1-\\rho\$ and vanishes as \$\\rho\\to1\$\.",
...
it does not establish a reduction in ranking bias."""),
        ...
        (r"This is a computational reduction of the exponential coalition family, and any\s+reduction of redundancy bias relative to the canonical family is an empirical\s+claim rather than a theorem\.",
...
of redundancy bias follows from Proposition~\ref{prop:redund}."""),
        ...
        (r"A redundant group now contributes two aggregate questions.*?stay in place\.",
...
is documented and their influence is assessed by an ablation analysis."""),
        ...
        (r"The adaptive construction of \\S\\ref\{sec:probes\} extends F2 from exact to.*?exponential family of subcoalition\s+aggregates\.",
...
optional cluster-level questions whose influence is explicitly tested."""),
        ...
        # 7.9 Final sentence
        (r"The clustering threshold \$\\theta\$ is the only correlation parameter and is\s+reported with a sensitivity check, so the approximate redundancy control remains\s+as auditable as the exact family axioms\.",
...
an axiomatic redundancy-control guarantee."""),
        ...
        # 8. Near duplicates
        (r"Near-duplicate probes are handled by the correlation clustering above, with the\s+realised within-cluster correlation \$\\rho\$ reported alongside the threshold\s+\$\\theta\$\.",
...
examined through ablation and overlap sensitivity analyses."""),
        ...
        # 19. Proof of K=1
        (r"For \$K=1\$ a profile is a single number, and A2 states\s+\$d\\preceq_1 e\\iff d\\le e\$,",
...
$d\preceq_1 e$. Hence $d\preceq_1 e\iff d\le e$,""")
    ...
    "experiments_new.tex": [
        ...
        (r"the adaptive construction of \\S\\ref\{sec:probes\} recovers exactly one redundant\s+cluster and summarises the environmental pair with a single mean and a single max\s+aggregate, instead of letting redundant coalition aggregates proliferate, while\s+the two singletons are retained for separation\.",
...
concern. Its influence is therefore reported through the no-clustering ablation."""),
        ...
        (r"Removing the correlation clustering leaves the recommendation and the held-out\s+loss unchanged.*?liability on this instance\.",
...
settings."""),
        ...
        (r"Only the full adaptive family combines a discriminating, readable certificate\s+with competitive tail loss\.",
        ...
        (r"so the behaviour of LexPR on this instance comes from the declared structure of\s+the family rather than from lexicographic sorting alone\.",
        ...
        (r"A near-zero value indicates a flat certificate in which no declared probe\s+registers a binding concern\.",
        ...
        (r"re-evaluating LexPR over the box of plausible bounds, approximated by Monte Carlo\s+sampling.*?the sampled\s+necessary-winner set\.",
...
necessary-winner approximation."""),
        ...
        (r"The environmental singletons and cluster probes have zero disappointment under\s+the nominal active anchors\.",
        ...
        (r"They do not by themselves prove optimality without the rival profiles and audit\s+data\.",
...
for filename, reps in replacements.items():
    ...
    for i, (old_regex, new) in enumerate(reps):
        # We use re.sub with DOTALL and count=1
        new_text, count = re.subn(old_regex, new.replace("\\", "\\\\"), text, flags=re.DOTALL | re.MULTILINE)
        if count == 0:
            print(f"Warning: Regex not found in {filename} (Replacement {i+1}):\n{old_regex[:100]}...")
        else:
            text = new_text
