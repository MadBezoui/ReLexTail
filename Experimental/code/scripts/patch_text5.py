...
and the specified ASF baseline."""),
        ...
        # 3.3
        (r"LexPR is most appropriate when stakeholders can defend a finite set of monotone\s+questions.*?and probe family\.",
...
shortlist, criterion bounds, probe anchors, and probe family."""),
        ...
        # 17
        (r"\\item Retain only probes with \$q\^\{\\mathrm w\}-q\^\\star>\\varepsilon\$, and flag degenerate probes(?!\s+as)",
        ...
        # 31
        (r"Before final submission, the DOI and all scripts should be verified from a clean.*?JSON\} outputs\.",
...
profile.}"""),
        ...
        # 34
        (r"Criterion redundancy & Correlation cluster map and realised within-cluster correlation \$\\rho\$\.\s*\\\\",
        ...
        # 34.1
        (r"Explanation can be slower & Short binding-probe certificate plus a full audit certificate\.\s*\\\\",
...
sensitivity analyses."""),
        ...
        # 7.3
        (r"Redundancy is treated as positive correlation.*?must remain separate\.",
...
correlation coefficient."""),
        ...
        # 7.4
        (r"Thus the discrepancy induced by summarising a redundant cluster by its mean is.*?vanishes as \$\\rho\\to1\$\.",
...
it does not establish a reduction in ranking bias."""),
        ...
        # 7.5
        (r"This is a computational reduction of the exponential coalition family, and any.*?claim rather than a theorem\.",
...
of redundancy bias follows from Proposition~\ref{prop:redund}."""),
        ...
        # 7.6
        (r"A redundant group now contributes two aggregate questions.*?singletons stay in place\.",
...
is documented and their influence is assessed by an ablation analysis."""),
        ...
        # 7.8
        (r"The adaptive construction of \\S\\ref\{sec:probes\} extends F2 from exact to.*?exponential family of subcoalition aggregates\.",
...
optional cluster-level questions whose influence is explicitly tested."""),
        ...
        # 8
        (r"Near-duplicate probes are handled by the correlation clustering above.*?threshold \$\\theta\$\.",
...
$d\preceq_1 e$. Hence $d\preceq_1 e\iff d\le e$,""")
    ...
    "experiments_new.tex": [
        # 10
        (r"the adaptive construction of \\S\\ref\{sec:probes\} recovers exactly one redundant.*?retained for separation\.",
...
concern. Its influence is therefore reported through the no-clustering ablation."""),
        ...
        # 11.1
        (r"Removing the correlation clustering leaves the recommendation and the held-out.*?liability on this instance\.",
...
settings."""),
        ...
        # 14
        (r"re-evaluating LexPR over the box of plausible bounds, approximated by Monte Carlo.*?necessary-winner set\.",
