...
quality, and domain meaning of the criteria"""),
    ...
    (r"""With naive probe evaluation, computing the optimal class costs
$O(NKm+NK\log K+NK)$,""",
r"""Let $C_{\Q}$ denote the cost of evaluating all declared probes for one
alternative. Computing the optimal class costs
\[
O(NC_{\Q}+NK\log K+NK).
\]
For the canonical singleton, weighted-mean, and maximum probes evaluated
naively, $C_{\Q}=O(Km)$, giving
$O(NKm+NK\log K+NK)$,"""),
...
probability."""),
    ...
    (r"Otherwise the \emph{certified selector} returns the interval possible and necessary winner sets.",
...
files = ["main.tex", "theory_core.tex", "experiments_new.tex", "supplement.tex"]
...
for old_text, new_text in replacements:
    ...
    for filename in files:
        ...
        text_fixed, count = flex_replace(text, old_text, new_text)
        
        if count > 0:
            with open(filepath, "w") as f:
                f.write(text_fixed)
            found = True
            print(f"Applied replacement to {filename}")
            break
            
    if not found:
        print(f"Warning: Regex not found in ANY file for:\n{old_text[:80]}...")
